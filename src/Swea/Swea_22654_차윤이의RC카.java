...
public class Swea_22654_차윤이의RC카 {
	...
	static int startX, startY, endX, endY, commandLen, direction, n, result; 
	...
	public static void main(String[] args) {
        ...
        for (int test_case = 1; test_case <= T; test_case++) {
            ...
            int q = sc.nextInt(); //조종을 한 횟수 
            
            //로직
            StringBuilder sb = new StringBuilder();
            for(int i = 0; i < q; i++) {
            	result = 0;
            	commandLen = sc.nextInt(); //커맨드 길이 
            	String command = sc.next(); //커맨드
            	direction = 0; //방향 위로 설정 
            	//조종
            	sb.append(pilot(command) + " ");
            }
